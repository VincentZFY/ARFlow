...
def get_co_transforms_SSL(aug_args):
    transforms = []
    if aug_args.crop:
        transforms.append(RandomCropSSL(aug_args.para_crop))
    if aug_args.hflip:
        transforms.append(RandomHorizontalFlipSSL())
    if aug_args.swap:
        transforms.append(RandomSwapSSL())
    return ComposeSSL(transforms)
...
class ComposeSSL(object):
    ...
    def __call__(self, input, target,supervised):
        for t in self.co_transforms:
            input, target = t(input, target,supervised)
        return input, target
...
class RandomCropSSL(object):
    ...
    def __call__(self, inputs, target,supervised):
        h, w, _ = inputs[0].shape
        th, tw = self.size
        if w == tw and h == th:
            return inputs, target
...
class RandomHorizontalFlipSSL(object):
    """Randomly horizontally flips the given PIL.Image with a probability of 0.5
    """
    def __call__(self, inputs, target,supervised=False):
        if random.random() < 0.5:
            inputs = [np.copy(np.fliplr(im)) for im in inputs]
            if 'mask' in target:
                target['mask'] = [np.copy(np.fliplr(mask)) for mask in target['mask']]
