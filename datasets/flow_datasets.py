...
class ImgSeqDatasetSSL(Dataset, metaclass=ABCMeta):
    def __init__(self, root, n_frames, input_transform=None, co_transform=None,
                 target_transform=None, ap_transform=None,supervised_mode=False,supervised_target_transform=None):
        self.root = Path(root)
        self.n_frames = n_frames
        self.input_transform = input_transform
        self.co_transform = co_transform
        ...
        self.supervised_target_transform = supervised_target_transform
        self.samples = self.collect_samples()
    ...
    def __getitem__(self, idx):
        ...
        if self.target_transform is not None:
            for key in self.target_transform.keys():
                target[key] = self.target_transform[key](target[key])
        if self.supervised_target_transform is not None:
            for key in self.supervised_target_transform.keys():
                target[key] = self.supervised_target_transform[key](target[key])
...
class SintelSSL(ImgSeqDatasetSSL):
    def __init__(self, root, n_frames=2, type='clean', split='training',
                 subsplit='trainval', with_flow=True, ap_transform=None,
                 transform=None, target_transform=None, co_transform=None, supervised_mode=False, supervised_target_transform=None):
        self.dataset_type = type
        self.with_flow = with_flow
        ...
        self.subsplit = subsplit
        self.supervised_mode = supervised_mode
        self.training_scene = ['alley_1', 'ambush_5', 'bamboo_1', 'bandage_1', 'cave_2', 'market_2', 'shaman_2',
        ...
        super(SintelSSL, self).__init__(root, n_frames, input_transform=transform,
                                     ...
                                     ap_transform=ap_transform, supervised_mode=supervised_mode,
                                     supervised_target_transform=supervised_target_transform)
    ...
    def collect_samples(self):
        ...
        for flow_map in sorted((self.root / flow_dir).glob('*/*.flo')):
            ...
            if self.supervised_mode == True and scene not in self.supervised_scene:
                continue
            elif self.supervised_mode == False and scene not in self.unsupervised_scene:
                continue
